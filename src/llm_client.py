...
def generate_openai_compatible_report(
    ...
    if not api_key:
        raise ValueError("生成 LLM 报告需要填写 API Key。")
    if not model:
        raise ValueError("生成 LLM 报告需要填写 Model Name。")
    ...
    try:
        ...
        raise RuntimeError(f"LLM API 请求失败：HTTP {exc.code}。{body}") from exc
    ...
    try:
        ...
        raise RuntimeError(f"LLM API 返回格式不符合预期：{data}") from exc
